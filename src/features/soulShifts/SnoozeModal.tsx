...
type Props = {
  ...
  theme: Theme;
  onClose: () => void;
  /** Confirm fires with the selected `until` date. */
...
export function SnoozeModal({
  ...
  const handleConfirm = () => {
    ...
    until.setDate(until.getDate() + selectedDuration.days);
    // Note is captured in UI but not sent — backend snooze endpoint takes
    // only snoozed_until today. Kept for parity with Tend/Release reason
    // input rhythm; if BE adds a snooze_reason field later, plumb it
    // through onConfirm's signature.
    onConfirm(until);
  };
          ...
          >
            ...
            >
              ...
              <Text style={[styles.prompt, { color: ink(theme) }]}>
                Pause this shift for now?
              </Text>

              <Text style={[styles.sectionLabel, { color: inkFaint(theme) }]}>
                For how long
              </Text>
              <View style={styles.chipsRow}>
                {DURATIONS.map((d) => {
                  const sel = d.key === duration;
                  return (
                    <Pressable
                      key={d.key}
              ...
              <View style={styles.btnRow}>
                ...
                <Pressable
                  ...
                  accessibilityLabel={`Snooze for ${selectedDuration.label}`}
...
const styles = StyleSheet.create({
  ...
  // Body-length Edensor italic (so-6a7).
  prompt: {
